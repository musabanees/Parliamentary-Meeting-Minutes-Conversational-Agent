...
"""
    ...
    def __init__(self, params_path: str = "params.yaml"):
        ...
        logging.info("Initialized ParliamentaryEvaluator")
    ...
    def generate_questions(
        ...
        logging.info(
            f"Generating {self.num_questions_per_chunk} questions per node...")
        ...
        for node_id, text in tqdm(node_dict.items(), desc="Generating questions"):
            ...
            try:
                ...
                logging.error(
                    f"Error generating questions for node {node_id}: {e}")
                continue

        logging.info(
            f"Generated {len(queries)} total questions from {len(nodes)} nodes")
    ...
    def evaluate_retriever(
        ...
        for k in self.top_k_values:
            ...
            logging.info(
                f"top_{k}: Hit Rate={result_df['Hit Rate'].values[0]:.2f}, MRR={result_df['MRR'].values[0]:.3f}")
    ...
    def save_results(self, results_df: pd.DataFrame):
        ...
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        md_path = self.results_base_path / \
            f"retriever_evaluation_{timestamp}.md"
    ...
    def _generate_markdown_report(self, results_df: pd.DataFrame) -> str:
        ...
        report = f"""# Retriever Evaluation Results
...
"""
    ...
    def run(self):
        ...
        if len(nodes) == 0:
            logging.warning(
                "Dummy query returned 0 nodes, using Qdrant scroll...")
        ...
        if self.max_nodes and self.max_nodes < len(nodes):
            logging.info(
                f"Limiting to first {self.max_nodes} nodes (MAX_NODES_FOR_EVAL)")
        ...
        if self.dataset_path.exists():
            logging.info("=" * 80)
            logging.info(
                f"DATASET ALREADY EXISTS - Skipping question generation")
            logging.info(
                f"Loading existing evaluation dataset from {self.dataset_path}")
            logging.info("=" * 80)
            eval_dataset = EmbeddingQAFinetuneDataset.from_json(
                str(self.dataset_path))
            logging.info(
                f"Loaded {len(eval_dataset.queries)} questions from existing dataset")
        else:
            logging.info("=" * 80)
    ...
    def _get_nodes_from_qdrant(self, agent: ParliamentAgent) -> List[TextNode]:
        ...
        logging.info(
            f"Qdrant collection has {collection_info.points_count} points")
